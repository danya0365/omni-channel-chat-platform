...
### Stack & โครงสร้าง — ⚠️ ยังไม่เลือก (TBD)

ยังไม่ได้ตัดสินใจภาษา/framework/DB/queue. **เมื่อเคาะแล้ว** ให้:

1. เขียน ADR ใน `.claude/memory/decisions/` (`/new-adr`) บันทึกเหตุผล
2. อัปเดตส่วนนี้ให้เป็น stack จริง + โครงโฟลเดอร์
3. แตก scoped rule เฉพาะ stack ใน `.claude/rules/` (เช่น `backend-*.md`, `frontend-*.md`, per-channel)

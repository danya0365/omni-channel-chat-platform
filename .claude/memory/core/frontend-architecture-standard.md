...
- **กัน God function ทั้ง repo** (ไม่ใช่แค่ inbox) — root `eslint.config.mjs` มี backstop `max-lines` 300 · `max-lines-per-function` 120 ·
  `complexity` 12 · `max-depth` 4 คุม backend (apps/api, packages, widget) · inbox ยกเว้น (config ตัวเองเข้มกว่า) · ปรัชญา: handler บาง → logic ไป domain/service
- **server-first เป็น default** (พี่เคาะ — EGA fetch เกือบทั้งหมดฝั่ง server) — ดึง/mutate ฝั่ง server (RSC / server action **proxy → apps/api**
  เพราะ backend เป็น Fastify แยก ไม่ใช่ Supabase) · **client fetch = ข้อยกเว้นที่ต้อง justify** (`// client-fetch:` comment): realtime(WS)/optimistic/interaction ·
  **enforce = review** ไม่ใช่ tool-gate · prerequisite = ย้าย auth localStorage → httpOnly cookie (ตอนแตะ RSC จริง) · inbox realtime ปัจจุบัน = documented exception
